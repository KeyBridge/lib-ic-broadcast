...
-- 09/28/18 - physical data model v3.x
--            rename 'ca_station' table to facility
--            rename facility date fields to strings, parse in Java object
--            ised_bdbs.asdate function is deprecated
--            ised_bdbs.dms2dec function is deprecated
--              date and position fields are parsed and computed in software
-- 09/29/18 - set the physical data model version in the database name        v3
--
-- -----------------------------------------------------------------------------
-- CREATE THE DATABASE
-- -----------------------------------------------------------------------------
--
DROP DATABASE IF EXISTS ised_bdbs_3;
CREATE DATABASE ised_bdbs_3;
USE ised_bdbs_3;


-- -----------------------------------------------------------------------------
-- deprecated - date values are parsed in the Java object
--
-- Function asdate
...
-- -----------------------------------------------------------------------------
-- DEPRECATED
DROP function if EXISTS ised_bdbs_3.asdate;
DELIMITER //
CREATE FUNCTION ised_bdbs_3.asdate(col char(8))
  RETURNS DATE
  DETERMINISTIC
  READS SQL DATA
  BEGIN
...
-- -----------------------------------------------------------------------------
-- deprecated - latitude and longitude values are parsed and converted in software
...
-- -----------------------------------------------------------------------------
-- DEPRECATED
DROP function if EXISTS ised_bdbs_3.dms2dec;
DELIMITER //
CREATE FUNCTION ised_bdbs_3.dms2dec(col varchar(7))
  RETURNS double
  DETERMINISTIC
  READS SQL DATA
  BEGIN
...
CREATE TABLE `facility` (
  ...
  `latitude` char(6) NOT NULL,   -- ddmmss
  `longitude` char(7) NOT NULL,  -- dddmmss
  `alloc_zone` int(11),
  `ant_mode` char(1),
  `auto_prog` varchar(1),
  `bc_mode` char(1),
  ...
  `ifrbn_d` double,
  `ifrbn_n` double,
  `latitude2` char(6),    -- ddmmss
  `limit_code` varchar(8), 
  `longitude2` char(7),   -- dddmmss
  `network` char(4),
  `off_prec` char(1),
  `offset` char(1),
  `ok_dump` varchar(8),  -- date yyyyMMdd
  `overall_h` double,
  `par_rms_c` double,
  `par_rms_d` double,
  `par_rms_n` double,
  ...
  `scmo` char(1),
  `ss_code` char(5),
  `st_creat` varchar(8),  -- date yyyyMMdd
  `st_mod` varchar(8),    -- date yyyyMMdd
  `status1` char(2),
  `status2` char(2),
  `unattended` char(1),
  `usa_land` double,
...
CREATE TABLE `apatstat` (
  `calls_banr` varchar(32),  -- hide for InnoDB
  `patt_key` int(11) DEFAULT '0',
  `call_sign` varchar(12),
  `banner` char(2),
  PRIMARY KEY (`calls_banr`,`patt_key`), -- hide for InnoDB
...
--  KEY `call_sign_2` (`call_sign`,`banner`)
  ...
  FOREIGN KEY (call_sign, banner) REFERENCES facility (call_sign, banner) ON UPDATE CASCADE ON DELETE CASCADE
) ENGINE=MyISAM;
